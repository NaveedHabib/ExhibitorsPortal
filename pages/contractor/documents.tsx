...
export default function MyAccount() {
    ...
    const [GridPlan, SetGridPlan] = useState<string>();
    const [RiggingPlan, SetRiggingPlan] = useState<string>();

    useEffect(() => {
        if (loggedData) {
            axios.get(`${Globals.API_URL}Account/GetExhibitorProfile/${loggedData.type == "exhibitor" ? loggedData.id.toString() : loggedData.exhibitorId}`).then((r: any) => {
                ...
                SetGridPlan(dataModel.ElectricalGridPlan);
                SetRiggingPlan(dataModel.RiggingGridPaln);
            });
        }
    }, []);
                ...
                axios.post(`${Globals.API_URL}Account/UploadImage`, {
                    ...
                    .then((data: any) => {
                        if (data) {
                            ...
                            else if (e.target.name == "GridPlan") {
                                SetGridPlan(filename);
                            }
                            else if (e.target.name == "RiggingPlan") {
                                SetRiggingPlan(filename);
                            }
                            else {
    ...
    function updateProfile(e: any) {
        ...
        axios.post(`${Globals.API_URL}Exhibitor/ContractorUpdate/${loggedData?.type == "exhibitor" ? loggedData?.id.toString() : loggedData?.exhibitorId}`, ({
            "Design3D": File3DDesign,
            "TechnicalDrawing": FileTechnicalDrawing,
            "ElectricalGridPlan": GridPlan,
            "RiggingGridPaln": RiggingPlan,
                        ...
                        <form className='my-form' onSubmit={(e) => {
                            ...
                            <div className="row">
                                <div className="col-lg-6 mb-3">
                                    <label htmlFor="GridPlan" className='text-secondary'>Electrical Grid Plan*</label>
                                    <input className='form-control' type="file" name="GridPlan" onChange={fileUpload} accept=".png,.jpg,.jpeg,.pdf" />
                                    {
                                        GridPlan ? (
                                            <a href={GridPlan} className="text-primary" style={{ fontSize: "14px" }} target={"_blank"}>Click here to view Grid Plan</a>
                                        ) : ""
                                    }
                                </div>

                                <div className="col-lg-6 mb-3">
                                    <label htmlFor="RiggingPlan" className='text-secondary'>Rigging Plan*</label>
                                    <input className='form-control' type="file" name="RiggingPlan" onChange={fileUpload} accept=".png,.jpg,.jpeg,.pdf" />
                                    {
                                        RiggingPlan ? (
                                            <a href={RiggingPlan} className="text-primary" style={{ fontSize: "14px" }} target={"_blank"}>Click here to view Rigging Plan</a>
                                        ) : ""
                                    }
                                </div>
                            </div>
